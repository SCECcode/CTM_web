...
<head>
    ...
    <title>SCEC Community Thermal Model Explorer: User Guide</title>
</head>
<body>
<?php echo $header; ?>
...
<div class="container info-page-container scec-main-container guide">
    ...
    <div class="row">
        <div class="col-12">
            <figure class="ctm-interface figure float-lg-right">
                <img src="img/ctm-explorer.png" class="figure-img img-fluid" alt="Screen capture of CTM Explorer interface">
                <figcaption class="figure-caption">Screen capture of CTM Explorer interface</figcaption>
            </figure>
            <h3>SCEC Community Thermal Model Explorer Overview</h3>
	      <p>The CTM Explorer provides a user-friendly map-based interface to 
SCEC Community Thermal Models. It allows users to extract and visualize thermal properties from different models. The CTM Explorer currently has the functionality to produce various visualizations of the models including: 2D horizontal slices, 2D vertical cross sections, 1D vertical profiles, and individual 0D points from the extracted data.</p>
              <p>The pages on this site are the <a href="<?php echo $host_site_actual_path; ?>">CTM explorer page</a>, this user guide, <a href="disclaimer">a disclaimer</a>, and a <a href="contact">contact information</a> page.</p>
              <p>The main interface is on the <a href="<?php echo $host_site_actual_path; ?>">CTM Explorer Page</a>. The interactive map on the right displays the geographic extent of the selected CTM. On top of the map, there are selection buttons to overlay fault traces from the <a href="https://www.scec.org/research/cfm">SCEC Community Fault Model (CFM)</a>, and/or geological regions from <a href="https://www.scec.org/research/gfm">Geological Framework Model (GFM)</a> from the <a href="https://www.scec.org/research/crm">Community Rheology Model (CRM)</a>. In the top right corner of the interactive map, there is a pull-down menu that allows the base map to be changed. By default, the map shown is <a href="https://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer">ESRI Topographic</a>, but several other base map options are provided.</p>
...
<h3>Querying CTMs</h3>
		<p>The CTM Explorer is designed to provide quick access to a wide range of CTMs. These CTMs greatly differ in resolution and methods of construction. We have designed the CTM Explorer to automatically extract approximately 10,000 data points for each model query regardless of the source model resolution. Thus, the extracted data is likely interpolated from the source models. For this reason, we do not recommend using data extracted from the CTM Explorer in simulations. The CTM Explorer is instead designed to provide tools to quickly generate high quality visualizations of the CTMs and extract data for users to use with their own visualization codes. 
</p>
<h3>Model Selection</h3>
		<p>The boundary of the selected model is displayed on the map and additional model details are provided in the gray rectangle below the search buttons.</p>
<h3>Profile Types</h3>
                <p>The CTM Explorer currently has the functionality to produce various visualizations of the models including: 2D horizontal slices, 2D vertical cross sections, 1D vertical profiles, and individual 0D points from the extracted data. Once the profile type has been selected, the geographic range can either be manually typed into the boxes, or the ranges can be gathered from mouse clicks on the map. For 2D horizontal slices, the range can be selected by clicking and dragging on the map. For 2D vertical cross sections users can click on the map at the desired starting point and then clicked again on the ending point to set the geographic range. Once the geographic range has been selected, the depth ranges can be specified by filling in the necessary values (in meters) in the boxes. Once everything has been specified, click on “Extract Data” to start data extraction. Warning: CTMs are often very large and data extraction for some models may take several minutes. Once extraction is complete links will show up in the list directly below the Explorer interface. To return to the initial view with no selection, click the "Rest All" button.</p>
...
<h3>Plotting Extracted Data</h3>
		...
		<p><b>Query Points:</b> When selected, the extracted data points are plotted, which is useful to better understand the resolution of the plotted data.  All plots should contain approximately 10,000 data points.</p>
		...
		<p><b>Plot cities:</b> When selected, selected California and Nevada cities are plotted on maps. Note that this function is only optimized for California and Nevada regional maps and may not produce optimal results for larger maps (e.g., national maps).</p>
