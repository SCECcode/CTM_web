...
// leaflet layer for ctm model boundaries
...
// mpblob is what ctm_query returns
// [ { "uid": uid, "mp": mpblob }, ... ]
var ctm_mp_list=[];
...
function remove_all_models() {
  ctm_model_list.forEach(function(element) {
      var l=element['layer'];
...
function load_a_model(name, order) {
   var cnt=ctm_model_list.length;
   var i;
   for(i=0;i<cnt;i++) {
     var t=ctm_model_list[i];
     if(t['model'] == name ) {
       if(t['visible']==0) {
          t['visible']=1; 
          t['oidx']=order;
...
function make_all_model_layer() {
   ...
   for(i=0;i<cnt;i++) {
      ...
      ctm_model_list.push({"model": name, "layer": layer, "visible": 0, "oidx":0 });
   }
}

// can be "ctmh" or "ctmh,ctmsi"
function load_selected_model(modelstr) {
   // special case,  if ctm_model_ctm btn is enabled.. clear it
...
function insert_materialproperty(uid, mp) {
   ctm_mp_list.push( { "uid":uid, "mp":mp });
}
...
function get_materialproperty(target_uid) {
  var cnt=ctm_mp_list.length;
  for(var i=0; i<cnt; i++) {
    var element=ctm_mp_list[i];
...
function find_meta_from_list(target_uid) {
   var found=0;
   ctm_meta_list.forEach(function(element) {
...
{
   var found=0;
   ctm_layer_list.forEach(function(element) {
     if ( element['uid'] == target_uid )
        found=element;
   });
   return found;
...
function remove_all_layers() {
   ctm_layer_list.forEach(function(element) {
     ...
     switch (type)  {
       case POINT_ENUM: removeFromList(ctm_point_list,uid); break;
       case LINE_ENUM: removeFromList(ctm_line_list,uid); break;
       case PROFILE_ENUM: removeFromList(ctm_profile_list,uid); break;
       case AREA_ENUM: removeFromList(ctm_area_list,uid); break;
     };
...
function remove_a_layer(uid) {
   ...
   if(t) {
     ...
     var idx = ctm_layer_list.indexOf(t);
     if (idx > -1) {
       ctm_layer_list.splice(idx, 1);
     }
  }
}
...
function load_a_layergroup(uid,type,group,highlight) {
   ...
   ctm_layer_list.push({"uid":uid, "type":type, "group": group,"highlight":highlight});
   return 1;
}
...
function toggle_a_layergroup(uid) {
   ...
   if(found) {
      ...
      if(h==EYE_NORMAL) {
        ...
        $('#ctm_layer_'+uid).addClass('ctm-active');
      } else if (h==EYE_HIGHLIGHT) {
        hide_layergroup(group);
        found['highlight']=EYE_HIDE;
        $('#ctm_layer_'+uid).removeClass('ctm-active');
        $('#ctm_layer_'+uid).removeClass('glyphicon-eye-open');
        $('#ctm_layer_'+uid).addClass('glyphicon-eye-close');
      } else if (h==EYE_HIDE) {
        unhighlight_layergroup(group);
        found['highlight']=EYE_NORMAL;
        $('#ctm_layer_'+uid).addClass('glyphicon-eye-open');
        $('#ctm_layer_'+uid).removeClass('glyphicon-eye-close');
      }
      } else {
...
function add_bounding_profile(uid,a,b) {
  ...
  let t=ctm_profile_list;

  if(checkInList(ctm_profile_list,uid)!= undefined) {
