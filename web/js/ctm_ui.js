...
// show description 
    if(description.length > 350 || b_description != null) {
        ...
        if(b_description == null) {
          $("#ctm-model-description").html("<b>Description: </b><button class=\"btn btn-sm ctm-small-btn\" data-toggle=\"modal\" data-target=\"#modaldescription\"><span class=\"glyphicon glyphicon-expand\"></span></button>");
          } else {
              $("#ctm-model-description").html("<b>Description: </b>"+b_description+" <button class=\"btn btn-sm ctm-small-btn\" data-toggle=\"modal\" data-target=\"#modaldescription\"><span class=\"glyphicon glyphicon-expand\"></span></button>");
        }

        } else {
          $("#ctm-model-description").html("<b>Description:</b>"+description);
    }
...
// show author/references popup
    ...
    if(reflist.length != 0) {
        ...
        let tmp="";
        for(let i=0; i<cnt; i++) {
          astr=astr+rsp+"<button class=\"btn btn-sm ctm-small-btn\" data-toggle=\"modal\" data-target=\"#modalreference\" data-ref=\""+rlist[i]+"\">"+alist[i]+"</button>";
          tmp=tmp+rsp+reflist[i];
          rsp=" ";
        }
        if(cnt > 4 ) {
          $("#modalreferencebody").html("<div>"+astr+"</div>");
          $("#ctm-model-reference").html("<b>Reference: </b><button class=\"btn btn-sm ctm-small-btn\" data-toggle=\"modal\" data-target=\"#modalreference\"><span class=\"glyphicon glyphicon-expand\"></span></button>");
          } else {
              $("#ctm-model-reference").html(astr);
        }

        } else {
          $("#ctm-model-reference").html("");
    }
}
    ...
    for(i=0;i<sz;i++) {
       ...
       switch(keys[i]) {
          case 'csv':
              ...
              break;
          case 'gmtpdf':
              html=html+"<div style=\"margin-left:-6px\"><button id=\""+uid+"_show_btn\" class=\"btn btn-sm ctm-small-btn\" data-blob=\""+uid+"_state_blob\" data-toggle=\"modal\" data-target=\"#modalplotoption\"><span class=\"glyphicon glyphicon-picture\"></span></button>Plot data</div>";
              break;
          case 'gmtpng':
...
function insertMetaPlotResultTable(note,uid,str) {
    ctm_metaplottb_list.push( { uid:uid, blob:str });
...
function makeMetaPlotResultTable(note,uid,html) {
    ...
    if (ctm_metaplottb_list.length == 1) {
...
// insert at the end, row=table.insertRow(-1);
    ...
    let myhtml;
    if(hasLayer!=0) {
      myhtml="<td style=\"width:4px\"><button class=\"btn btn-sm ctm-small-btn\" title=\"toggle the layer\" onclick=toggle_a_layergroup(\""+uid+"\");><span value=0 id=\"ctm_layer_"+uid+"\" class=\"glyphicon glyphicon-eye-open\"></span></button></td><td style=\"width:10vw\">"+uid+"</td><td style=\"width:24vw\">"+html+"</td><td style=\"width:24vw\">"+note+"</td>";
      } else {
...
function downloadMPTable() {
    ...
    saveAsCSVBlobFile("CTM_",csvblob, uid);
}
...
function collapse_mp_table()
{
   ...
   if(v=="none") {
     ...
     $('#ctm_collapse_mp_btn').removeClass('ctm-active');
     $('#mpCollapseLi').text("Collapse");
     } else {
       elm.style.display='none';
       $('#ctm_collapse_mp_btn').addClass('ctm-active');
...
function collapse_mpr_table()
{
   ...
   if(v=="none") {
     elm.style.display='block';
     $('#ctm_collapse_result_btn').removeClass('ctm-active');
...
function movePlotview2() {
  var yourDOCTYPE = "<!DOCTYPE html>"; // your doctype declaration
  var copyPreview = window.open('about:blank', 'CTM_plot', "resizable=yes,scrollbars=yes,status=yes");
  var newCopy = copyPreview.document;
  newCopy.open();
  // remove header panel
  document.getElementById("plotoption-header").style.display="none";
